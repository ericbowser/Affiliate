      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              {product.tier && (
                <span className={{
                  budget: "bg-stone-100 text-stone-700",
          ...
          {product.verdict && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
              <h2 className="font-bold text-gray-900 mb-2 text-sm uppercase tracking-wider">
                Quick Verdict
              </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 rounded-2xl p-5">
                <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                  <span className="text-amber-600">✓</span> What we like
                </h3>
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-5">
                <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                  <span className="text-stone-400">✕</span> Watch out for
                </h3>
                <ul className="space-y-2">
          ...
          {(product.ownerPros || product.ownerCons) && (
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-2">What owners say</h2>
          ...
          {(product.whoFor || product.whoSkip) && (
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-5">Is this the right pick for you?</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {product.whoFor && (
                  <div className="bg-white border border-gray-200 rounded-2xl p-5">
                    <h3 className="font-semibold text-gray-900 mb-2">Buy it if...</h3>
                    <p className="text-sm text-gray-600 leading-relaxed">{product.whoFor}</p>
                  </div>
                )}
                {product.whoSkip && (
                  <div className="bg-white border border-gray-200 rounded-2xl p-5">
                    <h3 className="font-semibold text-gray-900 mb-2">Skip it if...</h3>
                    <p className="text-sm text-gray-600 leading-relaxed">{product.whoSkip}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-stone-800 rounded-2xl p-8 text-center">
            ...
            <p className="text-stone-400 text-sm mb-6">{product.price} · {product.bestFor}</p>
            <a
              href={product.url}
              target="_blank"
        ...
        <div className="space-y-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 lg:sticky lg:top-6">
            <div className="text-2xl font-bold text-amber-700 mb-1">{product.price}</div>
            ...
            <div className="space-y-3 text-sm border-t border-gray-100 pt-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-700 text-right max-w-[140px]">{product.bestFor}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Tier</span>
                <span className="text-gray-700 capitalize">{product.tier}</span>
              </div>
            </div>
          </div>
          ...
          {related.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              <h3 className="font-bold text-gray-900 mb-4">Also in {categoryName}</h3>
